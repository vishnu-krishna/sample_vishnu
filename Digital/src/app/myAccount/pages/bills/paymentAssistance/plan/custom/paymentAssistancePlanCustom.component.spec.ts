...
import { NowMock, PaymentExtensionStateMockService, InstalmentPlanOptionsMockService } from '../../../../../services/mock';
...
import { PaymentAssistancePlanCustomModule } from './paymentAssistancePlanCustom.module';
...
import { PaymentAssistancePlanCustomAmountModel } from './customAmount/models/customAmountModel';

describe('Payment Assistance Plan Custom Component', () => {
    // Mock dates
    ...
    let comp: PaymentAssistancePlanCustomComponent;
    let fixture: ComponentFixture<PaymentAssistancePlanCustomComponent>;
    let de: DebugElement;
    ...
    const monthlyOptionMaxStartDate = '2018-03-18';
    const contractNumber2 = '121213';
    ...
    beforeEach(() => {
        ...
        TestBed.configureTestingModule({
            ...
            providers: [
                ...
                { provide: IPaymentSchemeApi, useClass: PaymentSchemeApiStubService },
        ...
        fixture = TestBed.createComponent(PaymentAssistancePlanCustomComponent);
        comp = fixture.componentInstance;
        de = fixture.debugElement;

        router = TestBed.get(Router);
        activatedRoute = TestBed.get(ActivatedRoute);
        this.paymentSchemeApiStubService = TestBed.get(IPaymentSchemeApi);
    ...
    describe('onContinueButtonClicked()', () => {
        ...
        describe(`when an instalment plan is selected with instalmentAmount: '${instalmentSuggestion1.instalmentAmount}' and numberOfInstalments: '${instalmentSuggestion1.numberOfInstalments}' and continue is clicked`, () => {
            ...
            beforeEach(() => {
                ...
                comp.ngOnInit();
                fixture.detectChanges();
                comp.customAmountModel.amount = 100;
            });
            ...
            describe('when instalmentPlanSummaryService.initInstalmentPlanSummary() is called', () => {
                ...
                describe('and does not return an error', () => {
                    ...
                    beforeEach(() => {
                        ...
                        activatedRoute.snapshot.queryParams['contractAccountNumber'] = '1200000000';
                        activatedRoute.snapshot.queryParams['contractNumber'] = '1211111111';
                    ...
                    it('should navigate to the confirm screen', () => {
                        ...
                        expect(router.navigate).toHaveBeenCalledWith([`/bills/paymentassistance/plan/confirm/custom/${accountNumber}/${contractNumber}/${frequencyOptionFortnightly.value.toLowerCase()}/${formattedTodayDate}/${instalmentSuggestion1.instalmentAmount}`],
    ...
    describe('onCustomAmountFormValid()', () => {
        beforeEach(() => {
            comp.ngOnInit();
        });
        const amount = 100;
        describe(`when isFormValid: 'true', amount: ${amount} is passed`, () => {
            beforeEach(() => {
                comp.onCustomAmountFormValid({ isFormValid: true, amount: amount });
            });
            it(`should set comp.continueButtonEnabled to 'true'`, () => {
                expect(comp.continueButtonEnabled).toBeTruthy();
            });
            it(`should set customAmountModel.amount to '${amount}'`, () => {
                expect(comp.customAmountModel.amount).toBe(amount);
            });
        });

    });

    describe('onCustomAmountKeyUpEnter()', () => {
        it('should call onContinueButtonClicked()', () => {
            spyOn(comp, 'onContinueButtonClicked');
            comp.onCustomAmountKeyUpEnter();
            expect(comp.onContinueButtonClicked).toHaveBeenCalled();
        });
    });
    ...
    describe('Chat button', () => {
        it('should show chat button', () => {
            ...
            // assert
            const chatButton = fixture.debugElement.query(By.css('[chatButtonId=LPPaymentAssistanceCustom]'));
            expect(chatButton).toBeTruthy();
        });
    });

    describe('when paymentSchemeApi.submitPaymentArrangementInstalmentPlan() is called', () => {

        describe('and returns an error', () => {

            beforeEach(() => {

                fuelChipServiceInitSpy.and.returnValue(Observable.of(classifiedFuelChips));
                accountServiceGetAccountsSpy.and.returnValue(Observable.of(accounts));
                instalmentPlanOptionsServiceGetInstalmentOptions.and.returnValue(Observable.throw('error'));
                spyOn(dataLayerService, 'pushPaymentAssistanceErrorEvent');
    ...
    describe('when onContinueButtonClicked is called', () => {

        describe('and returns an error', () => {

            beforeEach(() => {
                comp.routeParamsModel = <PaymentAssistancePlanOptionsRouteParamsModel> {
                    accountNumber: accountNumber,
                    contractNumber: contractNumber,
                    cancelDestinationUrl: cancelDestinationUrl
                };

                comp.optionsFrequencyModel = <PaymentAssistancePlanOptionsFrequencyModel> {
                    dateOptions: dateOptionsWeekly,
                    selectedFrequencyOption: frequencyOptionWeekly,
                    frequencyOptions: frequencyOptions,
                    dateOptionsDisabled: false,
                ...
                comp.customAmountModel = <PaymentAssistancePlanCustomAmountModel> {
                    frequency: 'weekly',
                    amount: 40,
                    amountInput: '40',
                    instalmentMinAmount: 10,
                    instalmentMaxAmount: 100,
                    disabled: true,
            ...
            it('should set continueButtonLoading property to true', () => {
                expect(comp.continueButtonLoading).toBeFalsy();
            });
        });
