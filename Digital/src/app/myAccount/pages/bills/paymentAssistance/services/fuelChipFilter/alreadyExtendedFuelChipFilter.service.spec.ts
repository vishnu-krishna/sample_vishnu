...
describe('Already extended fuel chip filter service', () => {
    ...
    beforeEach(() => {
        ...
        testData = [
            new FuelChipDataModel('123', '111', MauiFuelChipFuelType.Electricity, null, new PaymentExtensionContractEligibility('111', false, PaymentExtensionIneligibilityReasons.None, totalAmountDue), MauiFuelChipFuelContext.Bill),
            ...
            new FuelChipDataModel('345', '333', MauiFuelChipFuelType.Gas, null, new PaymentExtensionContractEligibility('333', false, PaymentExtensionIneligibilityReasons.AlreadyExtended, totalAmountDue), MauiFuelChipFuelContext.Bill),
            new FuelChipDataModel('456', '444', MauiFuelChipFuelType.Electricity, null, new PaymentExtensionContractEligibility('444', false, PaymentExtensionIneligibilityReasons.AlreadyExtended, totalAmountDue), MauiFuelChipFuelContext.Bill)
        ];
    });
    ...
    it('should set primary status message to total to pay with formatted amount', async(() => {
        sut.filter(testData).subscribe((results) => {
            expect(results[1].statusMessage[0].primaryMessage).toBe('Total to pay: $363.30');
        });
    }));

    it('should set secondary status message to extended due date', async(() => {
        sut.filter(testData).subscribe((results) => {
            expect(results[0].statusMessage[0].secondaryMessage).toBe(`You've already set up payment assistance for this bill`);
        });
    }));
