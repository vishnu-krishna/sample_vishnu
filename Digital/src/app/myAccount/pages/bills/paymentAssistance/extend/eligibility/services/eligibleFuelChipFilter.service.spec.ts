...
describe('Eligible fuel chip filter service', () => {
    ...
    it('should include contracts when isEligible is true, exclude when false', async(() => {
        nowStub.date = (): moment.Moment => moment('2027-10-15');

        let testData: FuelChipData[] = [
            new FuelChipData('123', '111', MauiFuelChipFuelType.Gas, null, new PaymentExtensionContractEligibility('111', true, undefined, 1.00, new Date('2027-10-15')), MauiFuelChipFuelContext.Bill),
            new FuelChipData('234', '222', MauiFuelChipFuelType.Gas, null, new PaymentExtensionContractEligibility('222', true, undefined, 2.00, new Date('2027-10-15')), MauiFuelChipFuelContext.Bill),
            new FuelChipData('345', '333', MauiFuelChipFuelType.Gas, null, new PaymentExtensionContractEligibility('333', false, undefined, 3.00, new Date('2027-10-15')), MauiFuelChipFuelContext.Bill)
        ];
    ...
    it('should set primary status message to total to pay with formatted amount', async(() => {
        ...
        let testData: FuelChipData[] = [
            new FuelChipData('123', '111', MauiFuelChipFuelType.Gas, null,
    ...
    it('should set secondary status message to due date when bill is due in the future', async(() => {
        let today =  moment('2017-09-13');
        let dueDate = today.clone().add(1, 'days').toDate();
        nowStub.date = (): moment.Moment => today;

        let testData: FuelChipData[] = [constructFuelChipDataWithDueDate(dueDate)];
    ...
    it('should set secondary status message to due date when bill is due today', async(() => {
    ...
    it('should set secondary status message to "day" (singular) overdue when bill is due in the past', async(() => {
        let today = moment('2017-09-14');
        let dueDate = today.clone().add(-1, 'days').toDate();
        nowStub.date = (): moment.Moment => today;
    ...
    it('should set secondary status message to "days" (plural) overdue when bill is due in the past', async(() => {
        let today = moment('2017-09-14');
    ...
    function constructFuelChipDataWithDueDate(dueDate: Date): FuelChipData {
        return new FuelChipData('123', '111', MauiFuelChipFuelType.Gas, null, new PaymentExtensionContractEligibility('111', true, undefined, 123.45, dueDate), MauiFuelChipFuelContext.Bill);
    }
});
